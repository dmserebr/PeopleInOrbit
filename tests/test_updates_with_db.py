...
def test_updater_if_command_called_between_daily_updates(mocker):
    ...
    astronauts_data = json.loads(load_resource('test_astronauts_data.json'))
    data_new = astronauts_data[:-2]
    data_old = astronauts_data[1:]
    ...
    db_access.store_data_with_ts({'astronauts': data_old}, utcnow - timedelta(hours=1))

    # command with fetches new data on the same day
    mocker.patch.object(process_commands.content_generator, 'get_people_in_orbit', return_value=(None, data_new))
    process_commands.process_people_in_orbit(Message(user, chat, '/peopleinorbit'))

    # imitate call for tomorrow
    mocker.patch.object(updater_thread, 'get_astronauts_data', return_value=data_new)
    ...
    assert final_daily_data[0][1] == '{"astronauts": ' + json.dumps(data_old) + '}'
    ...
    assert final_daily_data[1][1] == '{"astronauts": ' + json.dumps(data_new) + '}'
